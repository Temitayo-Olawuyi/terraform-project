...
resource "aws_route53_zone" "Altschoolproject_zone" {
  name = var.domain_name
}

# create a terraform aws record set in route 53
resource "aws_route53_record" "project_domain_record" {
  zone_id = aws_route53_zone.Altschoolproject_zone.id
  name    = "terraform-test.${var.domain_name}"

  type    = "A"

  alias {
    name                   = aws_lb.Altschoolproject_lb.dns_name
    zone_id                = aws_lb.Altschoolproject_lb.zone_id
